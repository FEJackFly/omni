(() => {
    ...
    var Ie = (r) => oe(r, "__esModule", { value: !0 });
    var Me = (r, e) => {
        Ie(r);
        for (var t in e) oe(r, t, { get: e[t], enumerable: !0 });
    };
    var v = "3.4.14";
    function ae(r, e) {
        if (!r) throw new Error(e || "loaders.gl assertion failed.");
    }
    var g = {
        ...
        yt = g.self || g.window || g.global || {},
        gt = g.window || g.self || g.global || {},
        ne = g.global || g.self || g.window || {},
        bt = g.document || {};
    var D =
        ...
        At = (se && parseFloat(se[1])) || 0;
    function C(r, e = !0, t) {
        let o = t || new Set();
        if (r) {
            if (ie(r)) o.add(r);
            else if (ie(r.buffer)) o.add(r.buffer);
            else if (!ArrayBuffer.isView(r)) {
                if (e && typeof r == "object") for (let n in r) C(r[n], e, o);
            }
        }
        return t === void 0 ? Array.from(o) : [];
    }
    function ie(r) {
        return r
            ? r instanceof ArrayBuffer ||
                  (typeof MessagePort != "undefined" &&
                      r instanceof MessagePort) ||
    ...
    function x() {
        ...
        try {
            eval(
                "globalThis.parentPort = require('worker_threads').parentPort"
            ),
    ...
    var W = new Map(),
        m = class {
            ...
            static postMessage(e, t) {
                let o = { source: "loaders.gl", type: e, payload: t },
                    n = C(t),
                    s = x();
    ...
    function Te(r) {
        ...
        return (
            ...
            t.source.startsWith("loaders.gl")
        );
    }
    var V = {};
    Me(V, {
        readFileAsArrayBuffer: () => _e,
        readFileAsText: () => Se,
        requireFromFile: () => U,
        requireFromString: () => z,
    });
    var _e = null,
        ...
        z = null;
    var Oe = "latest",
        Ee = typeof v != "undefined" ? v : Oe,
        $ = {};
    async function M(r, e = null, t = {}) {
        return e && (r = ce(r, e, t)), ($[r] = $[r] || Le(r)), await $[r];
    }
    function ce(r, e, t) {
        ...
        return o[r]
            ...
            : D
            ? t.CDN
                ? (ae(t.CDN.startsWith("http")),
                  `${t.CDN}/${e}@${Ee}/dist/libs/${r}`)
                : I
                ? `../src/libs/${r}`
    ...
    async function Le(r) {
        if (r.endsWith("wasm")) return await (await fetch(r)).arrayBuffer();
        if (!D)
            try {
                return V && U && (await U(r));
            } catch {
                return null;
            }
        if (I) return importScripts(r);
        let t = await (await fetch(r)).text();
        return Fe(t, r);
    }
    function Fe(r, e) {
        if (!D) return z && z(r, e);
        if (I) return eval.call(ne, r), null;
    ...
    function Pe(r, e) {
        return new Promise((t, o) => {
            let n = Ne++,
                s = (l, d) => {
                    if (d.id === n)
                        switch (l) {
                            case "done":
                                m.removeEventListener(s), t(d.result);
                                break;
                            case "error":
            ...
            let i = { id: n, input: r, options: e };
            m.postMessage("process", i);
        });
    }
    async function ke({ loader: r, arrayBuffer: e, options: t, context: o }) {
        let n, s;
        if (r.parseSync || r.parse) (n = e), (s = r.parseSync || r.parse);
        else if (r.parseTextSync)
            (n = new TextDecoder().decode(e)), (s = r.parseTextSync);
        else throw new Error(`Could not load data with ${r.name} loader`);
        return (
            (t = {
                ...t,
                modules: (r && r.options && r.options.modules) || {},
                worker: !1,
            }),
            await s(n, { ...t }, o, r)
        );
    ...
    function ue(r, e) {
        if (!r) throw new Error(e || "loader assertion failed.");
    }
    var p = class {
        constructor(e, t) {
            ue(Array.isArray(e)),
        ...
        compareTo(e) {
            ...
            for (let t = 0; t < this.fields.length; ++t)
                if (!this.fields[t].compareTo(e.fields[t])) return !1;
            return !0;
        }
        select(...e) {
            let t = Object.create(null);
            for (let n of e) t[n] = !0;
            let o = this.fields.filter((n) => t[n.name]);
            return new p(o, this.metadata);
        }
        selectAt(...e) {
            let t = e.map((o) => this.fields[o]).filter(Boolean);
            return new p(t, this.metadata);
        }
        assign(e) {
            let t,
                o = this.metadata;
            if (e instanceof p) {
                let i = e;
                (t = i.fields),
                    (o = me(me(new Map(), this.metadata), i.metadata));
            } else t = e;
            let n = Object.create(null);
            for (let i of this.fields) n[i.name] = i;
            for (let i of t) n[i.name] = i;
    ...
    function Re(r) {
        ...
        for (let t of r)
            e[t.name] &&
                console.warn("Schema: duplicated field name", t.name, t),
                (e[t.name] = !0);
    }
    function me(r, e) {
    ...
    var pe = { HALF: 16, SINGLE: 32, DOUBLE: 64 },
        F = class extends u {
            constructor(e) {
                super();
                this.precision = e;
    ...
    var N = class extends F {
        ...
        J = class extends u {
            ...
            toString() {
                return `Date${(this.unit + 1) * 32}<${ve[this.unit]}>`;
            }
        };
    var fe = { SECOND: 1, MILLISECOND: 1e3, MICROSECOND: 1e6, NANOSECOND: 1e9 },
        X = class extends u {
            constructor(e, t) {
                super();
            ...
            toString() {
                return `Time${this.bitWidth}<${fe[this.unit]}>`;
            }
            get [Symbol.toStringTag]() {
                return "Time";
            }
    ...
    var Z = class extends u {
        ...
        toString() {
            return `Timestamp<${fe[this.unit]}${
                this.timezone ? `, ${this.timezone}` : ""
            }>`;
        }
    };
    var Ce = { DAY_TIME: 0, YEAR_MONTH: 1 },
        K = class extends u {
            constructor(e) {
                super();
                this.unit = e;
    ...
    function te(r, e, t) {
        let o = ye(e.value),
            n = t || ge(e);
        return new b(r, new k(e.size, new b("value", o)), !1, n);
    }
    function ge(r) {
        let e = new Map();
        return (
            "byteOffset" in r && e.set("byteOffset", r.byteOffset.toString(10)),
            "byteStride" in r && e.set("byteStride", r.byteStride.toString(10)),
            "normalized" in r && e.set("normalized", r.normalized.toString()),
            e
        );
    }
    function be(r, e, t) {
    ...
    function he(r, e, t) {
        let o = t ? Ae(t.metadata) : void 0;
        return te(r, e, o);
    }
    function Ae(r) {
        let e = new Map();
        for (let t in r) e.set(`${t}.string`, JSON.stringify(r[t]));
        return e;
    }
    var De = {
            POSITION: "POSITION",
        ...
        R = class {
            ...
            parseSync(e, t = {}) {
                ...
                o.Init(new Int8Array(e), e.byteLength),
                    this._disableAttributeTransforms(t);
                ...
                try {
                    ...
                    let l = this._getDracoLoaderData(s, n, t),
                        d = this._getMeshData(s, l, t),
                        f = G(d.attributes),
                        y = be(d.attributes, l, d.indices);
                    return {
                        loader: "draco",
                        loaderData: l,
            ...
            _getDracoAttributes(e, t) {
                ...
                for (let n = 0; n < e.num_attributes(); n++) {
                    ...
                    let l = this._getQuantizationTransform(s, t);
                    l && (o[s.unique_id()].quantization_transform = l);
                    let d = this._getOctahedronTransform(s, t);
                    d && (o[s.unique_id()].octahedron_transform = d);
                }
                return o;
            }
            _getMeshData(e, t, o) {
                let n = this._getMeshAttributes(t, e, o);
                if (!n.POSITION)
            ...
            _getTriangleStripIndices(e) {
                let t = new this.draco.DracoInt32Array();
                try {
                    return this.decoder.GetTriangleStripsFromMesh(e, t), ot(t);
                } finally {
                    this.draco.destroy(t);
                }
            }
            _getAttributeValues(e, t) {
                let o = Ke[t.data_type],
                    n = t.num_components,
                    i = e.num_points() * n,
                ...
                try {
                    let A = this.decoder.GetAttribute(e, t.attribute_index);
            ...
            _deduceAttributeName(e, t) {
                let o = e.unique_id;
                for (let [i, l] of Object.entries(t.extraAttributes || {}))
                    if (l === o) return i;
                let n = e.attribute_type;
                for (let i in De) if (this.draco[i] === n) return De[i];
                let s = t.attributeNameEntry || "name";
                return e.metadata[s]
                    ? e.metadata[s].string
                    : `CUSTOM_ATTRIBUTE_${o}`;
            }
            _getTopLevelMetadata(e) {
                let t = this.decoder.GetMetadata(e);
                return this._getDracoMetadata(t);
            }
            _getAttributeMetadata(e, t) {
                let o = this.decoder.GetAttributeMetadata(e, t);
            ...
            _getDracoMetadataField(e, t) {
                ...
                try {
                    this.metadataQuerier.GetIntEntryArray(e, t, o);
                    let n = rt(o);
                    return {
    ...
    function rt(r) {
        let e = r.size(),
            t = new Int32Array(e);
        for (let o = 0; o < e; o++) t[o] = r.GetValue(o);
        return t;
    }
    function ot(r) {
        let e = r.size(),
    ...
    const url = protocol + location.host + "/draco/";
    var at = "1.5.5",
        nt = "1.4.1",
        re = url,
        st = `${re}/draco_decoder.js`,
        it = `${re}/draco_wasm_wrapper.js`,
        ct = `${re}/draco_decoder.wasm`,
        Ar = `https://raw.githubusercontent.com/google/draco/${nt}/javascript/draco_encoder.js`,
        B;
    async function xe(r) {
        let e = r.modules || {};
        return (
    ...
    async function lt(r) {
        ...
        return (e = e || globalThis.DracoDecoderModule), await dt(e, t);
    }
    function dt(r, e) {
        let t = {};
        return (
            e && (t.wasmBinary = e),
            new Promise((o) => {
                r({ ...t, onModuleLoaded: (n) => o({ draco: n }) });
            })
        );
    }
    var we = { ...de, parse: ut };
    async function ut(r, e) {
        let { draco: t } = await xe(e),
            o = new R(t);
        try {
            return o.parseSync(r, e?.draco);
        } finally {
            o.destroy();
        }
    }
